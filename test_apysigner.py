...
class SignatureMakerTests(TestCase):
    ...
    def test_returns_payload_qs_sorted_by_dict_keys_and_vals(self):
        ...
        expected_qs = 'four_0=0&four_1=4&four_2=8&one=1&three=3&two=2'
        self.assertEqual(expected_qs, self.signer._encode_payload(payload))
    ...
    def test_returns_payload_qs_sorted_by_first_tuple_item_and_vals_when_item_repeats(self):
        ...
        expected_qs = 'one=1&two_0=2&two_1=dos&two_2=two'
        self.assertEqual(expected_qs, self.signer._encode_payload(payload))
    ...
    def test_get_signature_with_complex_non_unicode_payload(self):
        ...
        signature = get_signature(self.private_key, base_url, data)
        expected_signature = 'f64WESU4jxybEMn-3l4jmurrz9rBbXnrwe4JuHfvLh8='
        self.assertEqual(expected_signature, signature)
    ...
    def test_flattens_complex_example(self):
        ...
        new = self.signer._flatten(orig)
        self.assertEqual(new, {'a_b_c_0_d': 'e', 'a_b_c_1_f_0': 3, 'a_b_c_1_f_1': '1', 'a_b_c_1_f_2': '2'})
    ...
    def test_encodes_and_sorts_payload(self):
        ...
        for payload in payloads:
            ...
            self.assertEqual('a_b_c_0_f_0=3&a_b_c_0_f_1=1&a_b_c_0_f_2=2&a_b_c_1_a=1&a_b_c_1_d=e&a_b_c_1_w=3&a_b_c_1_z=2', encoded_payload)
            ...
            self.assertEqual('7G024wjiEKzmOsl-5_7I3cC6Tt3Rw5C6MAKyqt6w8F0=', result)
    ...
    def test_encodes_and_sorts_simple_payload(self):
        ...
        for payload in payloads:
            ...
            url_to_sign = url.path + '?' + url.query
            encoded_payload = self.signer._encode_payload(payload)
            self.assertEqual('a_0_8=3&a_1_1=0&a_1_9=2&b_0_1=0&b_0_9=2&b_1_1=z&b_1_7=4', encoded_payload)

            decoded_key = base64.urlsafe_b64decode(self.private_key.encode('utf-8'))
            signature = hmac.new(decoded_key, url_to_sign + encoded_payload, hashlib.sha256)
            result = base64.urlsafe_b64encode(signature.digest())
            self.assertEqual('o7t6Po2E-8IgXHtamgRZRe_U6yZqWSZGQnCJsfUPFRs=', result)
    ...
    def test_payload_with_different_value_types(self):
        ...
        url_to_sign = url.path + '?' + url.query
        encoded_payload = self.signer._encode_payload(payload)
        self.assertEqual('a_0=1&a_1_b=1&a_1_c=2&d=3&e=4', encoded_payload)

        decoded_key = base64.urlsafe_b64decode(self.private_key.encode('utf-8'))
        signature = hmac.new(decoded_key, url_to_sign + encoded_payload, hashlib.sha256)
        result = base64.urlsafe_b64encode(signature.digest())
        self.assertEqual('KcodMvl2gqmtNMUPx6-XzToxyTaJMLOcR5D3G37pSTo=', result)
    ...
    def test_sorts_inner_lists(self):
        base_url = 'http://imtapps.com/?a=1&b=2'
        url = urlparse.urlparse(base_url)
        url_to_sign = url.path + '?' + url.query
        payloads = [{"hi": ['a', 'b']}, {"hi": ['b', 'a']}]
        for payload in payloads:
            encoded_payload = self.signer._encode_payload(payload)
            self.assertEqual('hi_0=a&hi_1=b', encoded_payload)
            decoded_key = base64.urlsafe_b64decode(self.private_key.encode('utf-8'))
            signature = hmac.new(decoded_key, url_to_sign + encoded_payload, hashlib.sha256)
            result = base64.urlsafe_b64encode(signature.digest())
            self.assertEqual('W3gTbe2M3S4ugyOdAPqwuy-pRdF-PAvfeOD6qQwvBW0=', result)
    ...
    def test_big_example(self):
        base_url = 'http://imtapps.com/?a=1&b=2'
        payload = {"insured": {"id": 15288, "first_name": "CLEMENTINE", "last_name": "XXX", "name_2": "asdf", "address_1": "2885 130TH STREET", "address_2": "second address", "city": "YALE", "state": "IA", "zip_code": "502770000", "country": "USA"}, "property": {"id": 15280, "address": "Acres:     Qtr: SE Sec: 18 Twp: 81  Rng: 30", "city": "GUTHRIE", "state": "IA", "zip_code": "00000", "country": "USR", "description": "Dwelling"}, "mortgagee": {"id": 15288, "name_1": "WELLS FARGO BANK", "name_2": "FIRST AMERICAN", "address_1": "1224 1ST STREET", "address_2": "P.O. BOX 8", "city": "PERRY", "state": "IA", "zip_code": "502200000", "country": "GBR", "loan_number": "XXX", "mortgagee_type": "1ST"}, "agent": {"id": 15288, "agent_code": "yyy", "name_1": "asfd", "name_2": "SOMETHING", "address_1": "605 E. MAIN", "address_2": "P.O. BOX 99", "city": "xxx", "state": "IA", "zip_code": "999999999", "country": "XXR", "phone_number": "9999999999"}, "insurance_company": {"id": 15286, "naic_code": "00000", "name": "XYZ", "address_1": "988 WALNUT STREET, BOX 123", "address_2": "address two", "city": "XYA", "state": "IA", "zip_code": "500630624", "phone_number": "5159924121"}, "coverage_items": [
            {"id": 134081, "coverage_code": "O", "coverage_amount": 18, "deductible_amount": 19},
            {"id": 134080, "coverage_code": "O", "coverage_amount": 16, "deductible_amount": 17},
            {"id": 134079, "coverage_code": "O", "coverage_amount": 14, "deductible_amount": 15},
            {"id": 134078, "coverage_code": "G", "coverage_amount": 12, "deductible_amount": 13},
            {"id": 134077, "coverage_code": "F", "coverage_amount": 10, "deductible_amount": 11},
            {"id": 134076, "coverage_code": "E", "coverage_amount": 8, "deductible_amount": 9},
            {"id": 134075, "coverage_code": "D", "coverage_amount": 6, "deductible_amount": 7},
            {"id": 134074, "coverage_code": "C", "coverage_amount": 4, "deductible_amount": 5},
            {"id": 134073, "coverage_code": "B", "coverage_amount": 2, "deductible_amount": 3},
            {"id": 134072, "coverage_code": "A", "coverage_amount": 165000, "deductible_amount": 500}],
            "additional_information": {"id": 15288, "premium_amount": "99999.99", "increase_premium": "11111.11", "due_date": "2010-09-29", "escrow": False}, "id": 15318, "transaction_id": "201009290000002", "policy_id": "00183", "created": "2010-09-29", "reason": "AM", "activated": "2010-01-20", "effective": "2010-01-20", "expired": "2011-01-20", "policy_type_code": "19", "loaded": "2014-01-07T01:02:27.302", "received": None, "send": True, "batch_id": "a9ec0efe-7769-11e3-91e3-005056993634", "company": "Demo Company", "status": "", "error_message": ""}

        url = urlparse.urlparse(base_url)
        url_to_sign = url.path + '?' + url.query
        encoded_payload = self.signer._encode_payload(payload)
        self.assertEqual('activated=2010-01-20&additional_information_due_date=2010-09-29&additional_information_escrow=false&additional_information_id=15288'
                         '&additional_information_increase_premium=11111.11&additional_information_premium_amount=99999.99&agent_address_1=605+E.+MAIN&'
                         'agent_address_2=P.O.+BOX+99&agent_agent_code=yyy&agent_city=xxx&agent_country=XXR&agent_id=15288&agent_name_1=asfd&agent_name_2=SOMETHING&'
                         'agent_phone_number=9999999999&agent_state=IA&agent_zip_code=999999999&batch_id=a9ec0efe-7769-11e3-91e3-005056993634&company=Demo+Company&'
                         'coverage_items_0_coverage_amount=2&coverage_items_0_coverage_code=B&coverage_items_0_deductible_amount=3&coverage_items_0_id=134073&'
                         'coverage_items_1_coverage_amount=4&coverage_items_1_coverage_code=C&coverage_items_1_deductible_amount=5&coverage_items_1_id=134074&'
                         'coverage_items_2_coverage_amount=6&coverage_items_2_coverage_code=D&coverage_items_2_deductible_amount=7&coverage_items_2_id=134075&'
                         'coverage_items_3_coverage_amount=8&coverage_items_3_coverage_code=E&coverage_items_3_deductible_amount=9&coverage_items_3_id=134076&'
                         'coverage_items_4_coverage_amount=10&coverage_items_4_coverage_code=F&coverage_items_4_deductible_amount=11&coverage_items_4_id=134077&'
                         'coverage_items_5_coverage_amount=12&coverage_items_5_coverage_code=G&coverage_items_5_deductible_amount=13&coverage_items_5_id=134078&'
                         'coverage_items_6_coverage_amount=14&coverage_items_6_coverage_code=O&coverage_items_6_deductible_amount=15&coverage_items_6_id=134079&'
                         'coverage_items_7_coverage_amount=16&coverage_items_7_coverage_code=O&coverage_items_7_deductible_amount=17&coverage_items_7_id=134080&'
                         'coverage_items_8_coverage_amount=18&coverage_items_8_coverage_code=O&coverage_items_8_deductible_amount=19&coverage_items_8_id=134081&'
                         'coverage_items_9_coverage_amount=165000&coverage_items_9_coverage_code=A&coverage_items_9_deductible_amount=500&coverage_items_9_id=134072&'
                         'created=2010-09-29&effective=2010-01-20&error_message=null&expired=2011-01-20&id=15318&insurance_company_address_1=988+WALNUT+STREET%2C+BOX+123&'
                         'insurance_company_address_2=address+two&insurance_company_city=XYA&insurance_company_id=15286&insurance_company_naic_code=00000&'
                         'insurance_company_name=XYZ&insurance_company_phone_number=5159924121&insurance_company_state=IA&insurance_company_zip_code=500630624&'
                         'insured_address_1=2885+130TH+STREET&insured_address_2=second+address&insured_city=YALE&insured_country=USA&insured_first_name=CLEMENTINE&'
                         'insured_id=15288&insured_last_name=XXX&insured_name_2=asdf&insured_state=IA&insured_zip_code=502770000&loaded=2014-01-07T01%3A02%3A27.302&'
                         'mortgagee_address_1=1224+1ST+STREET&mortgagee_address_2=P.O.+BOX+8&mortgagee_city=PERRY&mortgagee_country=GBR&mortgagee_id=15288&'
                         'mortgagee_loan_number=XXX&mortgagee_mortgagee_type=1ST&mortgagee_name_1=WELLS+FARGO+BANK&mortgagee_name_2=FIRST+AMERICAN&mortgagee_state=IA&'
                         'mortgagee_zip_code=502200000&policy_id=00183&policy_type_code=19&property_address=Acres%3A+++++Qtr%3A+SE+Sec%3A+18+Twp%3A+81++Rng%3A+30&'
                         'property_city=GUTHRIE&property_country=USR&property_description=Dwelling&property_id=15280&property_state=IA&property_zip_code=00000&reason=AM&'
                         'received=null&send=true&status=null&transaction_id=201009290000002', encoded_payload)
        decoded_key = base64.urlsafe_b64decode(self.private_key.encode('utf-8'))
        signature = hmac.new(decoded_key, url_to_sign + encoded_payload, hashlib.sha256)
        result = base64.urlsafe_b64encode(signature.digest())
        self.assertEqual('V_ErAxav_Fh9zdhFD6_T3EypsSuJyID2_zSh8vBW8oM=', result)
